...
public class EntityCopyUtil {
    ...
    public static <T1, T2> void copyFieldValue(T1 sourceObj, T2 targetObj, Field field1, Field field2) {
        try {
            //1. 判断两个字段是否名称相同而且类型相同(兼容驼峰命令法和匈牙利命名法)
            if (equalFieldsName(field1, field2)
    ...
    /**
     * 判断两个字段的名称是否相同（兼容驼峰命令法和匈牙利命名法）
     *
     * @param field1
     * @param field2
     * @return
     */
    public static boolean equalFieldsName(Field field1, Field field2) {
        if (field1.getName().equals(field2.getName())) {
            return true;
        }
        String field1Name = field1.getName();
        String field2Name = field2.getName();
        if (hungarianToCamel(field1Name).equals(hungarianToCamel(field2Name))) {
            return true;
        }
        return false;
    }
    ...
    /**
     * 匈牙利命名法(Hungarian) 转 驼峰命令法(Camel)
     *
     * @param str
     * @return
     */
    public static String hungarianToCamel(String str) {
        String[] split = str.split("_");
        StringBuilder stringBuffer = new StringBuilder();
        for (String string : split) {
            if (!"_".equals(string) && StringUtils.isNotBlank(string)) {
                if (string.length() == 1) {
                    stringBuffer.append(string.toUpperCase());
                } else if (string.length() > 1) {
                    String character = string.substring(1);
                    String head = string.substring(0, 1);
                    stringBuffer.append(head.toUpperCase()).append(character);
                }
            }
        }
